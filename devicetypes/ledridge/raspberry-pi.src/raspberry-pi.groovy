...
preferences {
        input("ip", "string", title:"IP Address", description: "192.168.1.50", defaultValue: "192.168.1.50" ,required: true, displayDuringSetup: true)
...
metadata {
	definition (name: "Raspberry Pi", namespace: "Ledridge", author: "Ledridge") {
		capability "Polling"
		capability "Refresh"
	...
	tiles {
        ...
        valueTile("cpuPercentage", "device.cpuPercentage", inactiveLabel: false, decoration: "flat") {
        	state "default", label:'${currentValue}% CPU', unit:"Percentage", icon: "http://chart.apis.google.com/chart?cht=p&chs=120x120&chd=t:80,20&chco=ff000d&chf=bg,s,FFFFFF|c,s,FFFFFF", 
            backgroundColors:[
...
def parse(String description) {
    ...
    if (result.containsKey("cpu_temp")) {
    	log.debug "temp: ${result.cpu_temp}"
        log.debug "temp: ${celsiusToFahrenheit(result.cpu_temp.toDouble())} F"
    	sendEvent(name: "temperature", value: celsiusToFahrenheit(result.cpu_temp.toDouble()))
    }
